...
func (d *Pan123) newUpload(ctx context.Context, upReq *UploadResp, file model.FileStreamer, up driver.UpdateProgress) error {
	...
	buf := utils.BufferPoolGet(int(chunkSize))
	...
	for i := 1; i <= chunkCount; i += batchSize {
		...
		for j := start; j < end; j++ {
			...
			buf.Reset()
			_, err := utils.CopyWithBufferN(struct{ io.Writer }{buf}, file, curSize)
			if int64(buf.Len()) != curSize || err != nil {
				return fmt.Errorf("read chunk %d failed, expect =%d ,actual =%d ,error: %s", j, curSize, buf.Len(), err)
			}
			err = d.uploadS3Chunk(ctx, upReq, s3PreSignedUrls, j, end, bytes.NewReader(buf.Bytes()), curSize, false, getS3UploadUrl)
			if err != nil {
				return err
			}
